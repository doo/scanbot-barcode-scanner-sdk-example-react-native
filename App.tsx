...
const ListSource = [
  ...
  {
    id: '3',
    label: 'RTU-UI: Batch Barcode Scanner',
    action: async function (context: any) {
      if (!(await checkLicense())) {
        return;
      }
  ...
  {
    id: '5',
    label: 'Set accepted barcode types',
    action: async function (context: any) {
  ...
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        <Overlay
          visible={this.state.barcodeResultModalVisible}
          style={styles.overlay}
